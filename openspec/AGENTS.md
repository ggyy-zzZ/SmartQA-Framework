...
3. **提示词**：用户可见说明与 LLM system 内容应来自 `KnowledgeAssistantPrompts`、`SedimentationPlanPrompts`（沉淀方案专用）或 `qa.assistant.*` 配置，避免在业务类中散落写死某一商业系统名称。
...
10. **结构化沉淀方案流水线**：`POST /qa/mysql/sedimentation/pipeline` 在元数据目录上生成**可解析 JSON 方案**，可选二次模型 digest，再按方案启用的通路写入三库；**不**执行模型给出的任意 SQL/Cypher，物理形态仍为应用白名单。见 `openspec/design/schema-sedimentation-plan-pipeline.md`。